...
RegisterServerEvent('qb-restock:server:removemony', function(data, vehicle)
...
local discount = round(data.price * Config.discount)
    if Player.PlayerData.money.cash >= discount  then
        TriggerClientEvent("qb-restock:client:create_blip", src, data.vehicle)  
...
RegisterServerEvent('qb-restock:owncar', function(k, vehicle)
        local stock = nil
        local car = vehicle
        local isthere = false
